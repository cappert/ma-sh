...
import com.modernalchemists.mass.android.BillingService.RequestPurchase;
import com.modernalchemists.mass.android.BillingService.RestoreTransactions;
import com.modernalchemists.mass.android.Consts.PurchaseState;
 ...
 */
public class MassPurchaseObserver
{
    private static final String TAG = "MASS-JAVA - PurchaseObserver";
    ...
    public MassPurchaseObserver(Activity activity, Handler handler)
    {
        mActivity = activity;
        mHandler = handler;
    ...
    public void onBillingSupported(boolean supported)
    {
        ...
        if (supported) {
            ...
            Store.getInstance().showDialog(Store.DIALOG_BILLING_NOT_SUPPORTED_ID);
        }
    }
    ...
    public void onPurchaseStateChange(PurchaseState purchaseState, String itemId, long purchaseTime, String developerPayload)
    {
        ...
        {
        	if(developerPayload != null && developerPayload.compareTo(Store.REQUEST_TYPE_BUY_PRODUCT) == 0)
        		Store.getInstance().onProductBought();
        }
        
        if(purchaseState == PurchaseState.REFUNDED)
        {
        	Store.getInstance().onProductRefunded();
        }
    ...
    public void onRequestPurchaseResponse(RequestPurchase request, ResponseCode responseCode)
    {
        ...
        if (responseCode == ResponseCode.RESULT_OK) {
            ...
            Store.getInstance().onProductBought(); // TODO: remove this, send onBoughtResponse always (necessary due to buggy google API, purchase confirm response may never be sent) -> wtf?!
        } else if (responseCode == ResponseCode.RESULT_USER_CANCELED) {
            if (Consts.DEBUG) {
                Log.i(TAG, "user canceled purchase");
            }
            Store.getInstance().onTransactionFailed();
        } else {
            if (Consts.DEBUG) {
    ...
    public void onRestoreTransactionsResponse(RestoreTransactions request, ResponseCode responseCode)
    {
        if (responseCode == ResponseCode.RESULT_OK) {
            ...
            Store.getInstance().onRestoreTransactionsResponse();
        } else {
            if (Consts.DEBUG) {
